...
export async function PUT(request: NextRequest, ctx: Ctx) {
  ...
  // Only allow updating patches owned by the user
  const existing = await prisma.patch.findFirst({ where: { id, userId: session.user.id }, select: { id: true } })
...
export async function DELETE(_: NextRequest, ctx: Ctx) {
  ...
  // Only allow deleting patches owned by the user
  const existing = await prisma.patch.findFirst({ where: { id, userId: session.user.id }, select: { id: true } })
  if (!existing) return NextResponse.json({ error: 'Not found' }, { status: 404 })
