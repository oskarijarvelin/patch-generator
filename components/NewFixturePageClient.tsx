...
type ModeInput = { name: string; channelCount: string }
...
export default function NewFixturePage() {
  ...
  const [modes, setModes] = useState<ModeInput[]>([{ name: '', channelCount: '' }])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/fixtures', {
      ...
      body: JSON.stringify({ manufacturer, name, weight: Number(weight), powerConsumption: Number(power), isGlobal, modes: modes.map((m) => ({ name: m.name, channelCount: Number(m.channelCount) })) }),
    })
    setSaving(false)
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      <h1 className="text-3xl font-bold text-gray-900 mb-8">New Fixture</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={saving} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-6 py-2 rounded-lg text-sm font-medium">{saving ? 'Saving…' : 'Save Fixture'}</button>
